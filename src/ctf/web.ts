...
export async function httpRequest(
  ...
  const curl = await findCtfExecutable('curl', options.cwd)
  if (!curl) {
...
export async function httpDiff(
  args: {
    urlA: string
    urlB: string
    method?: string
  ...
  options: CommandOptions = {},
): Promise<CtfToolResultBase & { left: HttpRequestResult; right: HttpRequestResult; diff: Record<string, unknown> }> {
...
export async function probeWebBrowser(
  args: { url: string; captureScreenshot?: boolean },
  options: CommandOptions = {},
): Promise<BrowserProbeResult> {
  const base = emptyResult()
  const browser = await findCtfExecutable('chromium', options.cwd)
    ?? await findCtfExecutable('google-chrome-stable', options.cwd)
    ?? await findCtfExecutable('google-chrome', options.cwd)
  if (!browser) {
    base.status = 'missing_capability'
    base.limitations.push('No local Chromium or Google Chrome executable was found.')
    base.nextActions.push({ tool: 'ctf_tool_setup', args: { target: 'chrome_mcp' }, reason: 'Verify the installed mcp-chrome bridge; local headless Chromium remains the fallback.' })
    return {
      ...base,
      browser: null,
      url: args.url,
  ...
  if (args.captureScreenshot ?? true) {
    ...
    if (!shotCapture.ok) base.limitations.push(`Browser screenshot probe exited with ${shotCapture.exitCode ?? 'no status'}: ${shotCapture.error ?? shotCapture.stderr.trim()}`)
  }
  ...
  base.nextActions.push({ tool: 'mcp.chrome', args: { url: args.url }, reason: 'Use the configured mcp-chrome bridge for interactive tabs, DOM, console, network, cookies, and screenshots.' })
  base.nextActions.push({ tool: 'ctf_tool_setup', args: { target: 'chrome_mcp' }, reason: 'Verify mcp-chrome only if the external browser MCP is not responding.' })
  if (!domCapture.ok) base.limitations.push(`Browser DOM probe exited with ${domCapture.exitCode ?? 'no status'}: ${domCapture.error ?? domCapture.stderr.trim()}`)
  return {
    ...base,
...
export async function probeWebCapture(
  args: { listenHost?: string; listenPort?: number; webPort?: number },
  options: CommandOptions = {},
): Promise<WebCaptureProbeResult> {
  const base = emptyResult()
  const proxy = await findCtfExecutable('mitmweb', options.cwd)
    ?? await findCtfExecutable('mitmdump', options.cwd)
    ?? await findCtfExecutable('mitmproxy', options.cwd)
...
function curlArgs(args: HttpRequestArgs): string[] {
  ...
  for (const header of args.headers ?? []) argv.push('-H', header)
  if (args.body !== undefined) argv.push('--data-binary', args.body)
  argv.push(args.url)
  return argv
}

function parseCurlResponse(raw: string): HttpRequestResult['response'] {
...
function normalizeNumber(value: number | undefined, fallback: number, min: number, max: number): number {
  ...
  if (!Number.isFinite(value) || value < min || value > max) throw new Error(`number must be in range ${min}..${max}`)
  return value
}
