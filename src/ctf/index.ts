...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.tools.register(defineTool({
    name: 'ctf_tool_setup',
    description: 'Create an ordered human setup request for GDB/Pwndbg, IDA Pro, radare2, mcp-chrome, mitmproxy, Python CTF packages, or BlackArch packages. The human returns only logs, screenshots, or OCR text.',
    parameters: {
      target: {
        type: 'string',
        required: true,
        enum: ['gdb_pwndbg', 'ida_pro', 'r2', 'chrome_mcp', 'chrome_devtools_mcp', 'mitmproxy', 'python_ctf_env', 'blackarch_repo'],
