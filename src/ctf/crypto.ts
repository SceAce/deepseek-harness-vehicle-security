...
export interface CryptoProbeResult extends CtfToolResultBase {
  ...
  encodings: Array<{ type: string; confidence: string; decodedPreview?: string }>
  xorCandidates: Array<{ key: string; score: number; preview: string }>
}

export async function probeCryptoInput(
  input: { file?: ResolvedWorkspaceFile; text?: string },
): Promise<CryptoProbeResult> {
  const base = emptyResult()
  let artifact: CtfArtifactProfile | null = null
  let text = input.text ?? ''
  if (input.file) {
  ...
  if (encodings.length > 0) base.observations.push(`detected encoding candidates: ${encodings.map(item => item.type).join(', ')}`)
  if (xorCandidates.length > 0) base.observations.push(`ranked ${xorCandidates.length} single-byte XOR candidates.`)
...
function detectEncodings(text: string): Array<{ type: string; confidence: string; decodedPreview?: string }> {
  const compact = text.trim().replace(/\s+/g, '')
  const result: Array<{ type: string; confidence: string; decodedPreview?: string }> = []
  ...
  if (/^[A-Za-z0-9+/]+={0,2}$/.test(compact) && compact.length % 4 === 0) {
    try {
      ...
      if (decoded.length > 0) result.push({ type: 'base64', confidence: printableRatio(decoded) > 0.75 ? 'high' : 'low', decodedPreview: preview(decoded) })
    } catch {
...
function singleByteXorCandidates(bytes: Buffer, limit: number): Array<{ key: string; score: number; preview: string }> {
  const candidates: Array<{ key: string; score: number; preview: string }> = []
...
function englishScore(bytes: Buffer): number {
  let score = 0
  for (const byte of bytes) {
    const ch = String.fromCharCode(byte)
    if (/[etaoin shrdluETAOINSHRDLU]/.test(ch)) score += 2
    else if (/[a-zA-Z0-9{}_!?,.'"-]/.test(ch)) score += 1
    else if (byte === 9 || byte === 10 || byte === 13) score += 0.2
    else if (byte < 32 || byte > 126) score -= 4
  }
  return Number((score / Math.max(bytes.length, 1)).toFixed(4))
}

function printableRatio(bytes: Buffer): number {
  if (bytes.length === 0) return 0
  let printable = 0
  for (const byte of bytes) {
...
function preview(bytes: Buffer): string {
