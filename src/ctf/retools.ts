...
export async function queryRadare2(
  ...
  if (!capture.ok) base.limitations.push(`r2 exited with ${capture.exitCode ?? 'no status'}: ${capture.error ?? capture.stderr.trim()}`)
  return {
    ...base,
...
export function buildIdaScriptPlan(
  file: ResolvedWorkspaceFile,
  focus: string,
  execute = false,
  options: CommandOptions = {},
...
async function buildIdaScriptPlanImpl(
  file: ResolvedWorkspaceFile,
  focus: string,
  execute: boolean,
  options: CommandOptions,
...
function buildIdaScript(relativePath: string, focus: string): string {
