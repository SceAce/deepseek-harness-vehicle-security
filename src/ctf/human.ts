...
import { emptyResult, type CtfHumanOperation, type CtfHumanRequest, type CtfHumanReturnType, type CtfToolResultBase } from './types.js'
...
export function makeHumanRequest(
  input: Omit<CtfHumanRequest, 'acceptedReturnTypes' | 'returnContract'> & {
...
export function operationsFromLegacySteps(steps: string[]): CtfHumanOperation[] {
...
function normalizeHumanRequest(request: CtfHumanRequest): CtfHumanRequest {
  const rawOperations: CtfHumanOperation[] = request.operationOrder.length > 0
    ? request.operationOrder.map((operation, index) => ({
      ...operation,
      order: operation.order > 0 ? operation.order : index + 1,
    })) as CtfHumanOperation[]
  ...
  const acceptedReturnTypes: CtfHumanReturnType[] = request.acceptedReturnTypes.length > 0
