export function parseCanLog(text, options = {}) {
  ...
  for (const frame of frames) {
    const entry = counts.get(frame.id) ?? { id: frame.id, count: 0, channels: new Set() }
    entry.count += 1
    entry.channels.add(frame.channel)
    counts.set(frame.id, entry)
  }
  ...
  return {
    ...
    topIds,
    sampleFrames: frames.slice(0, 20).map(({ format: _format, ...frame }) => frame),
  }
}

function parseCandumpCompact(line) {
...
function parseCandumpColumns(line) {
...
function parseAsc(line) {
  const match = line.match(/^\s*(\d+(?:\.\d+)?)\s+(\d+)\s+([0-9A-Fa-f]+)x?\s+(Rx|Tx)\s+d\s+\d+\s*((?:[0-9A-Fa-f]{2}\s*)*)/i)
  if (!match) return null
  return frame('asc', match[1], match[2], match[3], match[5], match[4])
}

function frame(format, timestamp, channel, id, data, direction = null) {
