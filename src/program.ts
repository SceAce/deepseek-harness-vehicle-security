import { triageArtifact, type ArtifactTriageResult } from './artifact.js'
...
export async function analyzeProgram(
  ...
  if (format === 'elf') {
    ...
    if (readelf) {
      ...
      elf = parseElfMetadata(
        header.ok ? header.stdout : '',
        programHeaders.ok ? programHeaders.stdout : '',
        dynamic.ok ? dynamic.stdout : '',
        symbols.ok ? symbols.stdout : '',
        artifact.fileType,
      )
      ...
      for (const [label, capture] of Object.entries({ header, programHeaders, dynamic, symbols })) {
        if (!capture.ok) limitations.push(`readelf ${label} exited with ${capture.exitCode ?? 'no status'}: ${capture.error ?? capture.stderr.trim()}`)
  ...
  let interestingStrings: InterestingString[] = []
  ...
  if (strings) {
    ...
    if (!capture.ok) limitations.push(`strings exited with ${capture.exitCode ?? 'no status'}: ${capture.error ?? capture.stderr.trim()}`)
    if (capture.stdout.includes('...[truncated') || capture.stderr.includes('...[truncated')) {
...
export function parseElfMetadata(
  header: string,
...
export function parseUndefinedSymbols(symbols: string): string[] {
...
export function classifyStrings(output: string, maxStrings = 80): InterestingString[] {
  const result: InterestingString[] = []
  ...
  for (const line of output.split(/\r?\n/)) {
    ...
    if (tags.length === 0 || seen.has(value)) continue
    seen.add(value)
...
function detectFormat(fileType: string | null): ProgramAnalysisResult['format'] {
...
function elfObservations(elf: ElfMetadata, imports: string[]): ProgramObservation[] {
  return [
    {
      id: 'E-002',
      category: 'platform',
...
function buildConclusions(
  format: ProgramAnalysisResult['format'],
  elf: ElfMetadata | null,
  imports: string[],
  strings: InterestingString[],
  observations: ProgramObservation[],
): ProgramConclusion[] {
  const conclusions: ProgramConclusion[] = [{
    id: 'C-001',
...
export function buildValidationPlan(
  ...
  strings: InterestingString[],
  observations: ProgramObservation[],
): { hypotheses: ProgramHypothesis[]; validationSteps: ProgramValidationStep[] } {
  const hypotheses: ProgramHypothesis[] = []
  const validationSteps: ProgramValidationStep[] = []
  ...
  const matches = (set: Set<string>) => [...set].filter(name => lowerImports.has(name)).map(name => lowerImports.get(name)!)
  ...
  function addHypothesis(title: string, rationale: string, evidenceIds: string[], tool: string, actions: string[], successCriteria: string[]): void {
  ...
  if (execution.length > 0) addHypothesis(
    'An externally influenced value may reach a process-execution sink.',
    `Imported execution functions: ${execution.join(', ')}. Import presence alone does not show a controllable call path.`,
    [importEvidence],
    'IDA Pro MCP or radare2',
    [`Locate every cross-reference to ${execution.join(', ')}.`, 'Trace each argument backward to configuration, IPC, network, CAN/UDS, file, or constant sources.', 'Inspect quoting, allowlists, and direct exec argument construction.'],
    ['A reproducible path from a defined input to the sink is present, or every call site is shown to use constants/validated values.'],
  )
  if (memory.length > 0) addHypothesis(
    'A parser path may use a length-sensitive memory operation.',
    `Imported memory/string functions: ${memory.join(', ')}. Their use may be bounded and correct.`,
    [importEvidence],
    'IDA Pro MCP plus GDB/Pwndbg',
    [`Find cross-references to ${memory.join(', ')} and identify source/destination bounds.`, 'Set breakpoints on the relevant call sites and replay a recorded local input fixture.', 'Record lengths, destination capacity, return path, and any crash/sanitizer evidence.'],
    ['A call-site proof establishes safe bounds, or a minimized local input demonstrates an out-of-bounds condition.'],
  )
  if (network.length > 0) addHypothesis(
    'The program exposes or consumes a network-facing message path.',
    `Imported network functions: ${network.join(', ')}.`,
    [importEvidence],
    'IDA Pro MCP, radare2, and strace',
    [`Map cross-references to ${network.join(', ')} into listener/client initialization and message handlers.`, 'Run with a recorded test configuration and trace network syscalls in an isolated environment.', 'Correlate endpoint, framing, authentication, and parser functions.'],
    ['Static xrefs and a syscall trace agree on endpoint role and the first parser function.'],
  )
  if (credentialStrings.length > 0 && stringEvidence) addHypothesis(
    'Credential-like text may identify embedded defaults, field names, or authentication logic.',
    `Tagged strings include ${credentialStrings.slice(0, 5).map(item => `${item.offset}:${item.value}`).join(', ')}.`,
    [stringEvidence],
    'IDA Pro MCP or radare2',
    ['Resolve cross-references for each credential-tagged string.', 'Classify each use as UI text, field name, test data, format string, or secret material.', 'Inspect the surrounding authentication/configuration data flow.'],
    ['Each indicator is classified by xref and context; secret material requires byte-level evidence rather than keyword matching.'],
  )
  if (vehicleStrings.length > 0 && stringEvidence) addHypothesis(
    'The program likely implements a vehicle bus or diagnostic protocol path.',
    `Vehicle-tagged strings include ${vehicleStrings.slice(0, 5).map(item => `${item.offset}:${item.value}`).join(', ')}.`,
    [stringEvidence],
    'IDA Pro MCP, radare2, and capture correlation',
    ['Resolve xrefs to interface names, UDS services, CAN IDs, DoIP, SOME/IP, or MQTT topics.', 'Map initialization, receive, dispatch, authorization, and transmit functions.', 'Compare constants and branches with a saved CAN/ASC/PCAP trace.'],
    ['Code constants, dispatch logic, and recorded traffic agree on the protocol role and message direction.'],
  )
...
function stringTags(value: string): string[] {
  const tags: string[] = []
  if (/(?:https?:\/\/|mqtt|socket|connect|listen|bind\(|tls|ssl|websocket)/i.test(value)) tags.push('network')
  if (/(?:password|passwd|username|user_name|token|secret|api[_-]?key|credential|authorization)/i.test(value)) tags.push('credential')
  if (/(?:\/bin\/(?:sh|bash)|(?:^|\W)(?:system|popen|execv|execve|execl|execlp)\s*\()/i.test(value)) tags.push('execution')
  if (/(?:\bcan\d+\b|vcan|socketcan|isotp|iso-tp|\buds\b|doip|some\/?ip|obd|0x7e[0-9a-f])/i.test(value)) tags.push('vehicle')
  if (/(?:\/dev\/(?:tty|can|i2c|spi)|\/sys\/class|\/proc\/|\/etc\/)/i.test(value)) tags.push('device-or-config')
  if (/(?:assert|fatal|panic|debug|trace|error:|failed to)/i.test(value)) tags.push('diagnostic')
  return tags
}

function normalizeInteger(value: number | undefined, fallback: number, min: number, max: number, name: string): number {
