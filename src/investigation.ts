...
export interface InvestigationPlan {
  ...
  phases: Array<{ id: string; name: string; exitCriteria: string[] }>
  firstActions: InvestigationAction[]
  languagePlan: Array<{ language: string; useFor: string; avoidFor: string }>
  dataPlan: {
    directories: string[]
    stateFile: string
    namingRule: string
...
const KEYWORDS: Record<Exclude<InvestigationLane, 'unknown'>, RegExp> = {
...
export async function planInvestigation(
  ...
  file: ResolvedWorkspaceFile | null = null,
  options: InvestigationPlanOptions = {},
...
function rankLanes(text: string, file: ResolvedWorkspaceFile | null, artifact: ArtifactTriageResult | null): LaneCandidate[] {
  const scores = new Map<InvestigationLane, LaneCandidate>()
  for (const lane of LANES) scores.set(lane, { lane, score: 0, reasons: [] })
  const add = (lane: InvestigationLane, score: number, reason: string): void => {
  ...
  for (const [lane, regex] of Object.entries(KEYWORDS) as Array<[Exclude<InvestigationLane, 'unknown'>, RegExp]>) {
    if (regex.test(text)) add(lane, 3, 'objective or context contains lane-specific terms')
  }
  if (/(locate|find|trace|recover|implementation|handler|dispatcher|定位|追踪|实现)/i.test(text)) {
    if (KEYWORDS.firmware.test(text)) add('firmware', 4, 'objective asks to locate implementation inside firmware')
    else add('native-program', 2, 'objective asks for implementation-level tracing')
  }
  if (/(decode|reconstruct|timeline|traffic|capture|frame|signal|解码|重组|时序|流量|报文|信号)/i.test(text)) {
    if (KEYWORDS['can-uds'].test(text)) add('can-uds', 3, 'objective asks for protocol or traffic reconstruction')
    else add('network-protocol', 2, 'objective asks for protocol or traffic reconstruction')
  }

  if (file) {
    const extension = path.extname(file.relativePath).toLowerCase()
    if (['.asc', '.blf', '.dbc'].includes(extension) || /candump|can.*log/i.test(file.relativePath)) add('can-uds', 8, `artifact extension or name ${extension || file.relativePath}`)
    if (['.pcap', '.pcapng'].includes(extension)) add('network-protocol', 8, `capture extension ${extension}`)
    if (['.apk', '.aab', '.dex'].includes(extension)) add('android', 8, `Android extension ${extension}`)
    if (['.elf', '.so', '.exe', '.dll'].includes(extension)) add('native-program', 8, `program extension ${extension}`)
    if (['.bin', '.img', '.fw', '.hex', '.srec', '.s19', '.ubi', '.squashfs'].includes(extension)) add('firmware', 7, `firmware-like extension ${extension}`)
  }

  const fileType = artifact?.fileType ?? ''
  if (/ELF|PE32|Mach-O/i.test(fileType)) add('native-program', 10, `file identified as ${fileType}`)
  if (/Android package|Dalvik/i.test(fileType)) add('android', 10, `file identified as ${fileType}`)
  if (/pcap|capture file/i.test(fileType)) add('network-protocol', 10, `file identified as ${fileType}`)
  if (/filesystem|firmware|boot sector|u-boot|squashfs|ubifs/i.test(fileType)) add('firmware', 8, `file identified as ${fileType}`)
...
function investigationPhases(): InvestigationPlan['phases'] {
  return [
...
function actionsForLane(lane: InvestigationLane): InvestigationAction[] {
  const action = (
    id: string,
    phase: string,
    tool: string,
    ...
    nextIfPositive: string,
    nextIfNegative: string,
  ): InvestigationAction => ({ id, phase, tool, purpose, expectedSignal, nextIfPositive, nextIfNegative })
...
function languagesForLane(lane: InvestigationLane): InvestigationPlan['languagePlan'] {
  const result: InvestigationPlan['languagePlan'] = [
