...
export function routeCtfStart(input, artifact, audit) {
...
function inferCategory(text, artifact, url) {
    ...
    if (artifact) {
        ...
        if (/\bELF\b|Mach-O|shared object/i.test(fileType))
            return 'pwn';
        if (/PE32|MS-DOS executable/i.test(fileType))
            return 're';
        if (['.elf', '.so', '.dylib'].includes(extension))
            return 'pwn';
        if (['.exe', '.dll', '.sys', '.ocx'].includes(extension))
            return 're';
        if (['.apk', '.aab', '.dex'].includes(extension))
            return 're';
        if (['.pcap', '.pcapng', '.png', '.jpg', '.jpeg', '.gif', '.bmp', '.wav', '.mp3', '.zip', '.rar', '.7z', '.tar', '.gz'].includes(extension))
            return 'misc';
        if (['.py', '.js', '.java', '.c', '.cpp', '.go', '.rs', '.txt'].includes(extension) && /encrypt|decrypt|rsa|xor|flag|cipher/i.test(artifact.textSample ?? ''))
            return 'crypto';
    }
    return 'unknown';
}
function routeReasons(category, text, artifact, hasUrl, explicit) {
    const reasons = [];
    if (explicit)
        reasons.push(`category was explicitly set to ${explicit}`);
    if (hasUrl)
        reasons.push('url was provided, so web tooling is the first path');
    if (artifact?.fileType)
        reasons.push(`artifact fileType suggests ${category}: ${artifact.fileType}`);
    if (artifact?.extension)
        reasons.push(`artifact extension is ${artifact.extension}`);
    for (const [candidate, regex] of Object.entries(CATEGORY_KEYWORDS)) {
        if (regex.test(text))
            reasons.push(`prompt contains ${candidate} indicators`);
    }
    if (reasons.length === 0)
        reasons.push('no strong type signal; start with artifact profiling and tool audit');
    return reasons;
}
function firstTool(category, input, artifact, audit) {
    switch (category) {
        case 'pwn':
            return artifact
        ...
        default:
            if (artifact)
                return { tool: 'ctf_artifact_profile', args: { path: artifact.path } };
            if (hasCapability(audit, 'web.curl') && input.url)
                return { tool: 'ctf_http_request', args: { url: input.url, method: 'GET' } };
            return { tool: 'ctf_tool_audit', args: {} };
    }
}
function choicesForCategory(category, input, artifact, audit) {
...
function choice(audit, tool, args, reason) {
...
function humanRequests(input, category) {
    if (category === 'web' && !input.url)
