...
export async function discoverCtfPython(cwd = process.cwd()) {
    ...
    if (configured) {
        if (path.isAbsolute(configured) || configured.includes(path.sep)) {
            ...
            if (resolved)
                candidates.push({ executable: resolved, source: 'DSH_CTF_PYTHON' });
        }
    }
    ...
    if (pathPython)
        candidates.push({ executable: pathPython, source: 'PATH/python3' });
    const pathPythonLegacy = await findExecutable('python');
    if (pathPythonLegacy)
        candidates.push({ executable: pathPythonLegacy, source: 'PATH/python' });
    let selected = null;
