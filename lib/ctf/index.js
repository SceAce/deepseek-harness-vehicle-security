...
export function apply(ctx, config) {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            const file = await workspaceFile(config, exec, args.path);
            return profilePeArtifact(file, { ...commandOptions, signal: exec.signal });
        },
    }));
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            const file = await workspaceFile(config, exec, args.path);
            return profileAndroidArtifact(file, { ...commandOptions, signal: exec.signal });
        },
    }));
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            const file = await workspaceFile(config, exec, args.path);
            return profileMultiarchArtifact(file, { ...commandOptions, signal: exec.signal });
        },
    }));
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            const file = await workspaceFile(config, exec, args.path);
            return decompileAndroidArtifact(file, { ...commandOptions, signal: exec.signal });
        },
    }));
    ctx.tools.register(defineTool({
