...
export async function searchOneGadgets(file, args = {}, options = {}) {
...
async function resolveLibcTarget(file, libcPath, maxFileBytes) {
    if (libcPath?.trim()) {
...
function parseGadgets(rawOutput, maxResults) {
    if (!rawOutput)
        return [];
    const gadgets = [];
    let current = null;
    ...
    if (current && gadgets.length < maxResults)
        gadgets.push(current);
    return gadgets;
}
function isLibcName(name) {
...
function normalizeNear(value) {
    ...
    if (normalized.length === 0 || normalized.length > 200)
        throw new Error('near must contain 1..200 characters when provided');
    return normalized;
}
...
//# sourceMappingURL=one-gadget.js.map