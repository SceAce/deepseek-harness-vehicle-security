...
export async function profileSeccomp(file, args = {}, options = {}) {
...
function buildDumpArgv(targetPath, targetArgv, format, limit) {
...
function parseSyscalls(rules) {
    ...
    for (const rule of rules) {
        ...
        for (const match of rule.matchAll(/\b([a-z_][a-z0-9_]*)\s*\(/gi)) {
            if (match[1] && !['if', 'else', 'return'].includes(match[1].toLowerCase()))
                names.add(match[1].toLowerCase());
        }
    }
    return [...names].sort();
...
function normalizeLimit(value) {
    ...
    if (!Number.isInteger(value) || value < 1 || value > 32)
        throw new Error('limit must be an integer in range 1..32');
    return value;
}
