...
8. Choose `ctf_one_gadget` when a matching libc is available and a ret2libc or libc-base control-flow hypothesis is plausible.
9. Choose `ctf_seccomp_profile` when imports, strings, or runtime behavior suggest `prctl`, seccomp, sandboxing, or syscall restrictions.
10. Use `mcp.tavily` for libc, CVE, package-version, or debugger-reference lookup when required.
11. Use `ctf_tool_setup` with `gdb_pwndbg`, `one_gadget`, or `seccomp_tools` when the selected local backend is missing.
12. Use `ctf_python_exec` for Python helpers; it always uses `/home/source/tools/PyVenv/CTF/bin/python`.
13. Use `ctf_human_request` when the program needs a user-run service, device, GUI, or screenshot/log/OCR return.
...
libc gadget search -> ctf_one_gadget
...
- Prefer installed tooling like `checksec`, `gdb`, `ROPgadget`, `ropper`, `one_gadget`, and `pwntools`.
- Prefer `ctf_re_r2_query`, `ctf_pwn_gdb_probe`, `ctf_rop_search`, `ctf_one_gadget`, and `ctf_seccomp_profile` over generic shell commands when their callable backend is reported ready. Use shell only after the structured tool returns an explicit failure or leaves a concrete gap.
