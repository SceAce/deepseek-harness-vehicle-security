...
export async function profilePeArtifact(file, options = {}) {
    ...
    if (readobj) {
        ...
        if (capture.ok)
            base.observations.push('PE header, section, import, and export evidence collected with llvm-readobj.');
        else
            base.limitations.push(`llvm-readobj exited with ${capture.exitCode ?? 'no status'}.`);
    }
    else {
    ...
    if (objdump) {
        ...
        if (capture.ok)
            base.observations.push('PE private headers and loader metadata collected with llvm-objdump.');
        else
            base.limitations.push(`llvm-objdump exited with ${capture.exitCode ?? 'no status'}.`);
    }
    else {
...
export async function profileAndroidArtifact(file, options = {}) {
    const profile = await profileCtfArtifact(file, options);
    const base = baseFromArtifact(profile);
    const tools = [];
    ...
    if (aapt2) {
        ...
        if (capture.ok)
            base.observations.push('Android manifest/package metadata collected with aapt2.');
        else
            base.limitations.push(`aapt2 dump badging exited with ${capture.exitCode ?? 'no status'}.`);
    }
    else {
    ...
    for (const [name, executable, argv] of [
        ...
        if (!capture.ok)
            base.limitations.push(`${name} capability probe exited with ${capture.exitCode ?? 'no status'}.`);
    }
...
export async function profileMultiarchArtifact(file, options = {}) {
    const profile = await profileCtfArtifact(file, options);
    const base = baseFromArtifact(profile);
    const tools = [];
    ...
    for (const [name, executable, argv] of [
        ...
        tools.push({ name, executable, available: capture.ok, output });
        if (!capture.ok)
            base.limitations.push(`${name} architecture probe exited with ${capture.exitCode ?? 'no status'}.`);
    }
...
export async function decompileAndroidArtifact(file, options = {}) {
...
export async function executeMultiarchArtifact(file, args, options = {}) {
...
function baseFromArtifact(profile) {
    const base = emptyResult();
...
async function listRelativeFiles(root, workspaceRoot) {
    const result = [];
    async function visit(directory) {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory())
                ...
                result.push(path.relative(workspaceRoot, fullPath));
        }
    }
    await visit(root);
    return result.sort();
