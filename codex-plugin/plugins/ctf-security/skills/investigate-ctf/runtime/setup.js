...
function buildSetupRequest(target, context) {
    switch (target) {
        ...
        case 'ida_pro':
            return requestFromOperations('start_service', 'Install IDA Pro and prepare an IDAPython workflow', 'IDA is not configured locally, but the next RE step needs an IDAPython script-based workflow.', [
                {
                    order: 1,
                    kind: 'instruction',
        ...
        case 'chrome_devtools_mcp':
            return requestFromOperations('start_service', 'Configure Chrome DevTools MCP', 'The browser automation path needs a local Chrome DevTools MCP server.', [
                {
                    order: 1,
                    kind: 'command',
