...
export function parseHexBytes(input) {
    ...
    return Array.from({ length: normalized.length / 2 }, (_, index) => Number.parseInt(normalized.slice(index * 2, index * 2 + 2), 16));
}
export function decodeUds(input, stripIsoTp = true) {
    ...
    if (payload.length === 0)
        throw new Error('UDS payload is empty after ISO-TP decoding');
    const sid = payload[0];
    const base = {
        rawHex: toHex(raw),
    ...
    if (sid === 0x7F) {
        if (payload.length < 3)
            throw new Error('negative response requires SID and NRC bytes');
        return {
            ...base,
    ...
    if (requestSid === 0x10 && payload.length >= 2)
        result.sessionType = hexByte(payload[1] & 0x7F);
...
function unwrapIsoTp(bytes) {
    ...
    if (frameType === 0 && bytes[0] <= 0x0F) {
        ...
        if (length > bytes.length - 1)
            throw new Error('ISO-TP single-frame length exceeds payload');
